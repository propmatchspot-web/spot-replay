...
export async function updateUserPlan(userId: string, planTier: 'SPOT_BASIC' | 'SPOT_EXCLUSIVE') {
    ...
    if (userData?.referred_by) {
        ...
        if (price > 0) {
            try {
                const webhookUrl = process.env.AFFILIATE_WEBHOOK_URL || 'https://replay.propmatchspot.com/api/webhook/conversion'
...
export async function banUser(userId: string, isBanned: boolean) {
    ...
    */
    ...
    const supabase = await createClient()
    await supabase.from('users').update({ plan_tier: 'SPOT_BASIC' }).eq('id', userId)
