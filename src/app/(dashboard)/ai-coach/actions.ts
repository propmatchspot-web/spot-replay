...
async function checkAILimit(userId: string, type: 'vision' | 'chat') {
    ...
    // 2. UNLIMITED CHAT RULE (Spot Exclusive gets unlimited text chat)
    if (type === 'chat' && user.plan_tier === 'SPOT_EXCLUSIVE') {
    ...
    if (type === 'chat' && user.plan_tier === 'SPOT_BASIC') {
        limit = 3; // 3 chats/day for Spot Basic
    }

    if (user.plan_tier === 'SPOT_BASIC' || !user.plan_tier) {
        limit = type === 'chat' ? 3 : 1;
    }
