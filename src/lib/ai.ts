...
class OpenRouterProvider implements AIProvider {
  ...
  private getNextClient(): OpenAI {
    ...
    return new OpenAI({
      ...
      apiKey: key,
      defaultHeaders: {
        "HTTP-Referer": "https://replay.propmatchspot.com",
        "X-Title": "Spot Replay"
      }
    });
  }
...
class AIManager {
  ...
  private getSystemPrompt(context?: any, hasImage?: boolean): string {
    ...
    if (!hasImage) {
      return `
        IDENTITY:
        You are "Spot Buddy", a friendly and knowledgeable AI trading assistant at Spot Replay.
    ...
    return `
      IDENTITY:
      You are "Spot Buddy", a veteran Hedge Fund Portfolio Manager and ICT/SMC specialist at Spot Replay.
      ...
      **Entry Reasoning:** [Why this entry]
      ...
      ---
      *Analysis by Spot Buddy | ${new Date().toLocaleDateString()} | Stay Liquid.*
