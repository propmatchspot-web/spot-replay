...
export async function fetchDukascopyData(
    ...
    limit: number = 1000,
    endTime?: number, // ms
    startTime?: number // ms — if provided, use exact date range instead of range-based calc
): Promise<Candle[]> {
    try {
        ...
        // If startTime AND endTime are provided, use exact dates.
        // Otherwise, calculate from limit * interval.

        let fromDate: Date
        let toDate: Date

        if (startTime && endTime) {
            // Use exact user-selected dates
            fromDate = new Date(startTime)
            toDate = new Date(endTime)
            console.log(`[Dukascopy] Using exact date range from user selection`)
        } else {
            toDate = endTime ? new Date(endTime) : new Date()
            ...
            fromDate = new Date(toDate.getTime() - (rangeMs * 1.5))
        }
